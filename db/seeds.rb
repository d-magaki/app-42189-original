...
# 各部署の一般ユーザー
企画担当 = User.find_or_create_by!(email: "planner@example.com") do |user|
...
設計担当 = User.find_or_create_by!(email: "designer@example.com") do |user|
  user.employee_id = "EMP1002"
  user.user_name = "設計担当"
  user.department = "情報設計部"
  user.role = "一般社員"
  user.password = "password123"
  user.password_confirmation = "password123"
end

開発担当 = User.find_or_create_by!(email: "developer@example.com") do |user|
