...
4. 案件の一覧・詳細・編集（部署ごとに表示制御、削除、データインポートは管理者のみ）
-  ※各工程の開始・完了日などで新規登録　→　未着手　→　企画部　→　設計部　→　開発部　→　完了の流れをご確認ください  
5. 分析ページにて、案件の進行状況や依頼区分の割合などをグラフ表示（管理者のみ）
-  ※金額やステータスの変更でグラフ表示の変動をご確認ください
...
1. Excel のマクロを有効にする
-  ※有効にしても【Microsoftによりマクロの実行がブロックされました】が出た場合はデスクトップ等に別名保存してからやり直してください 
2. 「リスト作成」ボタンで行数を指定して案件を自動生成
-  ※リスト作成を間違った場合は削除ボタンでやり直してください
-  ※一部未入力カラム（企画IDなど）はマッピング用のためなので無視してください
3. Excelファイルを保存する  
